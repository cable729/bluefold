...
@Suite struct DeepLinkTests {
    ...
    @Test func formatsHashOnlyLink() {
        ...
        #expect(link.url().absoluteString == "bluefold://open?hash=abc123")
    }
    ...
    @Test func formatsPageAndPoint() {
        ...
        #expect(link.url().absoluteString == "bluefold://open?hash=abc123&page=42&x=72.0&y=590.5")
    }
    ...
    @Test func parsesOneBasedPageToZeroBasedIndex() {
        let link = DeepLink(url: URL(string: "bluefold://open?hash=h&page=1")!)
        #expect(link?.pageIndex == 0)
    }

    @Test func pointRequiresBothCoordinates() {
        let missingY = DeepLink(url: URL(string: "bluefold://open?hash=h&page=3&x=10")!)
        #expect(missingY?.pageIndex == 2)
        #expect(missingY?.point == nil)
    }

    @Test func pointWithoutPageIsIgnored() {
        let link = DeepLink(url: URL(string: "bluefold://open?hash=h&x=10&y=20")!)
    ...
    @Test func rejectsWrongSchemeHostAndMissingHash() {
        ...
        #expect(DeepLink(url: URL(string: "bluefold://close?hash=h")!) == nil)
        #expect(DeepLink(url: URL(string: "bluefold://open?page=3")!) == nil)
        #expect(DeepLink(url: URL(string: "bluefold://open?hash=")!) == nil)
    }

    @Test func rejectsInvalidPageNumbers() {
        #expect(DeepLink(url: URL(string: "bluefold://open?hash=h&page=0")!)?.pageIndex == nil)
        #expect(DeepLink(url: URL(string: "bluefold://open?hash=h&page=x")!)?.pageIndex == nil)
    }

    @Test func schemeIsCaseInsensitive() {
        #expect(DeepLink(url: URL(string: "Bluefold://OPEN?hash=h")!) != nil)
    }

    @Test func navEntryCarriesPageAndPoint() {
        let link = DeepLink(url: URL(string: "bluefold://open?hash=h&page=5&x=1.5&y=2.5")!)
