...
1. **Native Swift/SwiftUI/PDFKit over Electron/Tauri.** Every hard
   ...
   via CloudKit. Decided 2026-07-07.

2. **All logic in a root SwiftPM package; app shells are thin.** Enables
   `swift test` without Xcode, keeps UI-independent logic testable, and lets
   iOS reuse ReaderCore/persistence directly.
...
3. **Hand-authored pbxproj (objectVersion 77) with synchronized folder
   ...
   edits, so the project file avoids merge churn. XcodeGen was considered
   and skipped; revisit only if the project file becomes a conflict hotspot.
...
5. **Calibre is read-only, and the app has its OWN library.** Loose PDFs
   (downloads, coursework) must work without Calibre import friction. Calibre
...
10. **OCR inside the indexing actor, extractor_version-gated.** Search in
    scanned books is a hard requirement. Vision runs per page only when
...
12. **Notable UI fixes with root causes worth remembering:**
    sidebar dead-space = missing maxHeight fills in HSplitView
...
13. **Search never navigates while typing.** Results
...
14. **⌘-click opens the new tab ADJACENT to its source; same-book tabs get
    ...
    groups / sub-tab rows are a possible future step.

15. **Package.swift pre-declares all targets.** Independent milestones can
    be developed on parallel branches/worktrees without ever editing the
    manifest, so it never becomes a merge conflict hotspot.
...
16. **Sync (M15): snapshot-diff engine over plain CKDatabase ops, not
    ...
    pull-style `SyncTransport` protocol, and none of it could be live-tested
    until the one-time iCloud signing setup lands — blind code should be
    simple code. So: the engine exports full portable snapshots, diffs against a
